...
Flow (hoàn toàn trên Telegram):
  ...
  [Supervisor Agent]  ← create_react_agent + Ollama (gemma4)
      │
      ├─► call_weather_analyst(task)   → [Weather Agent] → get_weather API
      │
      └─► call_telegram_messenger(msg) → [Messenger Agent] → reply trên Telegram
...
Nhiệm vụ:
1. Dùng tool `get_weather` để lấy dữ liệu thời tiết cho địa điểm được yêu cầu.
2. Dữ liệu trả về có field `local_datetime` — đây là thời gian THỰC TẾ tại địa điểm đó \
(tính từ timestamp + timezone offset của OpenWeather). Luôn trích xuất và đưa vào kết quả phân tích.
3. Phân tích dữ liệu và viết nhận xét ngắn gọn bằng tiếng Việt:
   - Thời gian thực tế tại địa điểm (lấy từ `local_datetime`)
   - Nhiệt độ cảm nhận thực tế (nóng / mát / lạnh)
   - Độ ẩm và cảm giác oi bức
   - Tốc độ gió
   - Lời khuyên thực tế (ăn mặc, mang ô, ở trong nhà...)
4. Trả kết quả phân tích đầy đủ — KHÔNG gửi Telegram, đó là việc của agent khác.
"""

WEATHER_REPORTER_PROMPT = """Bạn là Weather Reporter — chuyên gia soạn và gửi báo cáo thời tiết qua Telegram.

Bạn sẽ nhận dữ liệu phân tích thời tiết từ Weather Analyst. Nhiệm vụ của bạn:
1. Soạn tin nhắn thân thiện (tối đa 200 từ) theo đúng cấu trúc sau:

   📍 *[Tên địa điểm]* — [emoji thời tiết phù hợp]
   🕐 *Thời gian thực tế tại địa điểm*: [lấy chính xác giá trị local_datetime từ dữ liệu analyst]
   🌡️ Nhiệt độ: *XX°C* (cảm giác như XX°C)
   💧 Độ ẩm: *XX%*
   💨 Gió: *X m/s*
   📝 [Nhận xét 1-2 câu về thời tiết]
   👗 [Gợi ý trang phục phù hợp]
   👉 _[Lời khuyên thực tế: mang ô, uống nước, tránh ra ngoài...]_

   ⏱️ _Dữ liệu thực tế lúc [local_datetime] theo giờ địa phương_

2. Dùng Markdown: *bold* cho số liệu, _italic_ cho lời khuyên và ghi chú.
3. Gọi tool `send_telegram_message` để gửi tin nhắn vừa soạn.
"""
...
Bạn có 3 công cụ:
- `call_weather_analyst`: Lấy và phân tích thời tiết cho một địa điểm.
...
Trường hợp 1 — Người dùng hỏi thời tiết hoặc gửi tên thành phố/địa điểm:
  Bước 1: Gọi `call_weather_analyst` với tên địa điểm.
  Bước 2: Gọi `call_weather_reporter` với toàn bộ kết quả từ bước 1.

Trường hợp 2 — Người dùng gửi tin nhắn hội thoại (chào hỏi, câu hỏi chung...):
  Bước 1: Gọi `call_chat_agent` với nội dung tin nhắn.
"""
...
def call_weather_analyst(task: str) -> str:
    ...
    Dùng tool này khi cần biết thông tin thời tiết của một địa điểm.

    Args:
        task: Mô tả nhiệm vụ cụ thể, bao gồm tên địa điểm cần tra cứu.

    Returns:
        Kết quả phân tích thời tiết chi tiết từ agent.
    """
